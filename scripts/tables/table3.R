...
# Lectura de datos
basecompleta<- read.csv(file="data/otrasbases/basecompleta.csv", header = TRUE,
                        sep = "\t" , encoding = "UTF-8")
## QUALTRICS
dataindividuo <- read.csv(file="data/otrasbases/baseindividuo.csv", header = TRUE,
                          sep = "\t" , encoding = "UTF-8")
...
```{r, echo=FALSE}
###################### MODELO PARA LAS TRES ZONAS ###############
##########################################################

library(lme4)


LongDat$fijacZonaA_normal<-LongDat$fijacZonaA/LongDat$totalFixa
LongDat$fijacZonaB_normal<-LongDat$fijacZonaB/LongDat$totalFixa
LongDat$fijacZonaC_normal<-LongDat$fijacZonaC/LongDat$totalFixa

fit2A <- lmer(LongDat$fijacZonaA_normal ~ LongDat$Asignada+LongDat$category+LongDat$IMPGlobal+
                LongDat$busca+LongDat$compra +
                (1 | LongDat$dataindividuo.n),  na.action = na.exclude,REML = FALSE)


fit2B <- lmer(LongDat$fijacZonaB_normal ~ LongDat$Asignada+LongDat$category+LongDat$IMPGlobal+ 
                LongDat$busca+LongDat$compra +(1 | LongDat$dataindividuo.n),na.action = na.exclude, REML = FALSE)



fit2C <- lmer(LongDat$fijacZonaC_normal ~ LongDat$Asignada+LongDat$category+LongDat$IMPGlobal+ 
                LongDat$busca+LongDat$compra +(1 | LongDat$dataindividuo.n), na.action = na.exclude,REML = FALSE)

## MODELO A

## Coeficientes
ctable <- coef(summary(fit2A))
...
Ajuste<-summary(fit2A)$AICtab
n<- summary(fit2A)$ngrps
...
Ajuste<-summary(fit2B)$AICtab
n<- summary(fit2B)$ngrps
ajusteb<-as.data.frame(Ajuste[c(1:2,5)])
ajusteb<- rbind(ajusteb,n)
row.names(ajusteb)<- c("AIC", "BIC", "Residual DF", "Subjects")
ajusteb<- format(round(ajusteb, 3), nsmall = 3)
...
Ajuste<-summary(fit2C)$AICtab
n<- summary(fit2C)$ngrps
ajusteb<-as.data.frame(Ajuste[c(1:2,5)])
ajusteb<- rbind(ajusteb,n)
row.names(ajusteb)<- c("AIC", "BIC", "Residual DF", "Subjects")
ajusteb<- format(round(ajusteb, 3), nsmall = 3)
...
knitr::kable(tablafinal,format = "html", booktabs = T, digits = c(3,3,3), align=c("r")) %>% 
  add_header_above(c(" " = 1, "Area A (Header)" = 3, "Area B (Menu)" = 3, "Area C (Products)" = 3)) %>%
  ...
  landscape() %>%
  footnote(general = "Linear mixed model fit by maximum likelihood", general_title = "Note: ",
           footnote_as_chunk = T)
